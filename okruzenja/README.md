...
Za potrebe praktičnog dela kursa potrebno je instalirati [Python3](https://www.python.org/downloads/) (preporučeno `Python 3.12`) i odgovarajuće biblioteke. Spisak svih potrebnih biblioteka sa odgovarajućim verzijama se nalazi u: 
...
2) Instalirati biblioteke (kroz *Terminal*):  
  ...
  b) `$ pip3.12 install -r requirements.txt` - ukoliko je na mašini instalirano više Python-a
...
2) Instalirati biblioteke (kroz *Command Prompt* ili *Powershell*):  
  ...
  b) `> pip3.12 install -r requirements_win.txt` - ukoliko je na mašini instalirano više Pyt
...
`$ cd sc-2025/`  
`$ python -m venv env` ili `$ python3.12 -m venv env`
...
```bash
$ pip install numpy scipy matplotlib pandas scikit-learn tensorflow opencv-contrib-python notebook
```
...
Uputstva za instalaciju možete pronaći [ovde](https://www.anaconda.com/docs/getting-started/anaconda/install).

Korisničku dokumentaciiju možete pronaći [ovde](https://www.anaconda.com/docs/main).
